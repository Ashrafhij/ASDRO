          ...
          <div className="w-full max-w-sm bg-white rounded-t-3xl p-5 pb-8 space-y-2 animate-slide-up shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            {navApps.map(({ key, labelKey, icon }) => (
              <button key={key} onClick={() => handleNavSelect(key, navLocation!)}
                ...
                <span className="w-9 h-9 rounded-xl bg-white shadow-sm border border-gray-100 flex items-center justify-center text-lg flex-shrink-0">{icon}</span>
                <span className="flex-1">{np[labelKey]}</span>
                {defaultNav === key && <span className="text-[10px] text-blue-500 font-medium bg-blue-50 px-2 py-0.5 rounded-full">Default</span>}
              </button>
            ))}
      ...
      <div className="space-y-4">
        ...
        {defaultNav && (
          <button onClick={clearDefault}
            ...
            🧭 Default: {np[navApps.find(a => a.key === defaultNav)!.labelKey]} — tap to change
          </button>
        )}
        ...
        {currentStop && (
          <div className="bg-gradient-to-br from-blue-600 to-indigo-600 rounded-2xl p-5 shadow-xl shadow-blue-200/40 space-y-4">
            <div className="flex items-center gap-1">
              ...
              {defaultNav && <span className="text-[10px] text-blue-300 ml-auto">🧭 {np[navApps.find(a => a.key === defaultNav)!.labelKey]}</span>}
            </div>
              ...
              <div className="flex-1 min-w-0 pt-1">
                ...
                {currentStop.customer.phone && (
                  <a href={`tel:${currentStop.customer.phone}`}
                    className="inline-flex items-center gap-1.5 mt-1.5 text-sm text-blue-100 bg-white/15 rounded-lg px-3 py-1.5 hover:bg-white/25 transition-colors w-max">
                    📞 {currentStop.customer.phone}
                  </a>
                )}
              </div>
                    ...
                    <div className="flex-1 min-w-0 pt-1">
                      <div className="flex items-center justify-between">
                        <span className={`text-sm font-semibold flex items-center gap-2 ${isComplete || isSkipped ? 'text-gray-500' : 'text-gray-900'}`}>
                          {wp.customer.name}
                          {wp.customer.phone && (
                            <a href={`tel:${wp.customer.phone}`} onClick={e => e.stopPropagation()}
